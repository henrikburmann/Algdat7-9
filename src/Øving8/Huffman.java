...
public class Huffman {
    ...
    public Huffman(File fileIn, File fileOut) throws IOException {
        root = huffmanTree(frequencyTable(fileIn));
    ...
    private static ArrayList<HuffmanNode> makeNodeList(){
        ArrayList<HuffmanNode> nodeList = new ArrayList<>();
        for (int i = 0; i < freq.length; i++) {
            if (freq[i] != 0){
                nodeList.add(new HuffmanNode((char) i, freq[i], null, null));
            }
        }
        return nodeList;
    }
    public byte[] deCompress(String inputFile) throws IOException {
        ...
        priorityQueue.addAll(makeNodeList());
        HuffmanNode tree = huffmanTree(freq);//Kan måtte gjøres om
        byte ch;
    ...
    private static Bitstring writeChar(HuffmanNode tree, Bitstring bitstring,
        ...
        if (tempTree.leftChild == null){
            long ch = tempTree.character;
            decompessedBytes.add((byte) ch);
            long temp = (long) ~(0);
